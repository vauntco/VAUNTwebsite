      ...
      <div className="container-v">
        ...
        <div className={`grid gap-4 sm:grid-cols-2 lg:grid-cols-3 ${showHeading ? 'mt-14' : ''}`}>
          {services.map((service, i) => {
            ...
            return (
              <FadeIn key={service.slug} delay={(i % 3) * 0.06} className="h-full">
                <TiltCard className="h-full">
                  <Link
                    to={`/services/${service.slug}`}
                    className="glass glow-hover group preserve-3d flex h-full flex-col p-6"
                  >
                    ...
                    <h3 className="mt-5 font-display text-xl font-semibold text-white" style={{ transform: 'translateZ(18px)' }}>
                      {service.name}
                    </h3>
